...
class InstanceManagerPanel(QWidget):
    ...
    def _build_ui(self):
        from PyQt6.QtWidgets import QFormLayout, QSizePolicy as QSP
        ...
        self._rt_status.setWordWrap(True)
        rt_l.addWidget(self._rt_status, 1)
        ...
        for label, action in [("▶ Start","start"), ("■ Stop","stop"), ("↺ Restart","restart")]:
            ...
            b.clicked.connect(lambda _c, a=action: self._container_action(a))
            btn_row.addWidget(b)
            if label == "▶ Start": self._start_btn = b
            elif label == "■ Stop": self._stop_btn = b
            else: self._restart_btn = b
        ...
        img_row.addWidget(self._image_combo, 1)
        cr_l.addLayout(img_row)
        ...
        cp_l.setSpacing(4)
        cp_l.addWidget(QLabel("Generate docker-compose.yml with PostGIS + services:"))
        ...
        for chk in (self._chk_featureserv, self._chk_tileserv, self._chk_pgadmin):
            cp_l.addWidget(chk)

        compose_btn_row = QHBoxLayout()
        compose_btn_row.setSpacing(4)
        for label, slot in [("👁 Preview", self._preview_compose),
                             ("💾 Save yml", self._save_compose),
                             ("▶ compose up -d", self._compose_up)]:
            b = QPushButton(label)
            b.setStyleSheet(_BTN_GREEN if "up" in label else _BTN)
            b.clicked.connect(slot)
            compose_btn_row.addWidget(b)
        ...
        self._compose_preview.setFixedHeight(130)
        self._compose_preview.setFont(QFont("Courier New", 9))
        ...
        self._saved_table.setHorizontalHeaderLabels([
            i18n.t("conn_name"), i18n.t("inst_type"),
            i18n.t("conn_host"), i18n.t("conn_database"),
        ])
        for c in range(4):
        ...
        self._saved_table.setFixedHeight(110)
        sv_l.addWidget(self._saved_table)
        ref2 = QPushButton("↻ Refresh")
        ref2.setFixedWidth(90)
